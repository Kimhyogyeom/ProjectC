...
- 적 기본 AI (플레이어 추적, 정지 거리 유지)
- 적 충돌 데미지 + 넉백 + 겹침 방지
- 적 HP / 플레이어 HP 관리
- HP 바 UI (World Space, 빌보드 효과)
- 웨이브 스폰 시스템 (웨이브마다 적 수 증가)

### 🔧 다음 작업 (PC에서 이어서)
1. **무적 시간** — 피격 후 N초간 무적 (PlayerHealth.cs 수정)
2. **적끼리 겹침 방지** — Enemy OnTriggerStay에서 Enemy 태그도 처리
3. **웨이브 UI** — 현재 웨이브 번호 화면에 표시
4. **레벨업 & 스킬 선택** — 경험치 획득 → 레벨업 → 스킬 3개 중 택 1
5. **플레이어 사망 처리** — 게임 오버 UI, 씬 재시작
