...
| 로그인 | 로비 | 강화 |
|:---:|:---:|:---:|
| ![로그인](Assets/Screenshots/login.PNG) | ![로비](Assets/Screenshots/lobby.PNG) | ![강화](Assets/Screenshots/upgrade.PNG) |

| 인게임 | 스킬 카드 |
|:---:|:---:|
| ![인게임](Assets/Screenshots/ingame.PNG) | ![스킬카드](Assets/Screenshots/skillcard.PNG) |
...
#### ⏳ 대기 중 (Google Play 개발자 계정 신분증 승인 대기 — 1~3일 소요)
- **신분증 승인 완료 후 해야 할 것**:
  1. Google Play Console 연락처 전화번호 인증 완료
  2. 내부 테스트 트랙에 AAB 업로드 → 출시
  3. 실기기에서 Google 로그인 테스트
  4. 로그인 성공 확인 후 **임시 로그인 코드 제거** (`LobbyManager.OnLoginClicked`의 TODO 주석 부분 + `FirebaseManager.SetTempLogin` 메서드)

#### ✅ Google Play 설정 완료 항목
- [x] Google Play Console 앱 등록 (com.kimhyogyeom.projectc)
- [x] Play Games Services 활성화 + 게시
- [x] OAuth 클라이언트 연결 (SHA-1 등록)
- [x] Firebase 새 계정 재설정 (Auth, Firestore, google-services.json)
- [x] Unity에서 GPGS Android Setup (App ID + Web Client ID)
- [x] 릴리즈 키스토어 생성 (user.keystore)
- [x] IL2CPP + ARM64 빌드 설정
- [x] 테스터 등록 (Play Games 서비스 + 내부 테스트)
- [ ] 개발자 계정 신분증 승인 (대기 중)
- [ ] 내부 테스트 AAB 출시
- [ ] 실기기 Google 로그인 테스트

#### 🔴 승인 후 즉시 해야 할 것
1. **내부 테스트 출시** — AAB 업로드 후 검토 및 출시
2. **실기기 Google 로그인 확인** — 로그인 → Firestore에 유저 데이터 분리 확인
3. **임시 코드 제거** — `SetTempLogin`, `OnLoginClicked` 임시 분기 제거
4. **로그인 UI 마무리** — 로그인 성공 시 이미지 비활성화 + 닉네임 표시 확인

#### 🚀 Google Play 스토어 출시 (내부 테스트 → 정식 출시)
5. **내부 테스트** — 테스터만 다운로드 가능, Google 로그인/데이터 동기화 검증
6. **비공개 테스트** — 소규모 유저 대상 테스트 (최대 수백 명)
7. **공개 테스트 (오픈 베타)** — 누구나 참여 가능, 스토어 등록 정보 완성 필요
8. **정식 출시** — Google Play 심사 통과 후 전체 공개
- 스토어 등록 정보 필요: 앱 아이콘, 스크린샷(최소 2장), 앱 설명, 개인정보처리방침 URL
- 콘텐츠 등급 설문 완료 필요
- 타겟 국가/지역 설정

#### UI / 시스템
9. **튜토리얼** — 첫 플레이 시 조이스틱/스킬 선택 안내
10. **로그인 UI 디자인** — 로그인 버튼 디자인 적용 (현재 기능만 구현)

#### 리텐션
11. **데일리 보상** — 출석 보상 시스템 (로비에서 매일 골드 지급)
12. **오프라인 보상** — 접속 안 한 시간만큼 골드 지급
13. **업적/도전과제** — 목표 달성 시 보상 (보스 처치 N회 등)

#### 콘텐츠
14. **소품 배치** — 기둥, 횃불 등 던전 오브젝트로 맵 채우기
15. **추가 스테이지** — 새로운 맵/테마

#### 수익화
16. **IAP** — 인앱 결제
17. **보석 상점** — 광고/IAP 연동
18. **LevelPlay Placement 등록** — 부활 광고 배치 추가 (수익 분석용, 필수 아님)

#### 장기
19. **장비 시스템** — 무기 3~5종, 강화/합성

### 📌 참고
- **임시 로그인**: 현재 Google Play 승인 대기로 인해 로그인 버튼 클릭 시 임시 로그인 처리 중 (익명 계정 기반). 승인 완료 후 반드시 임시 코드 제거 필요
- **Firebase 계정**: 새 계정(bazzirua@gmail.com)으로 재설정됨. 프로젝트 ID: `projectc-41776`
- **키스토어**: `user.keystore` — 비밀번호 분실 시 앱 업데이트 불가, 절대 삭제 금지
- **빌드 설정**: IL2CPP + ARM64 필수 (Google Play 64비트 요구사항)
