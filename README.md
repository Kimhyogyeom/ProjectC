...
#### 폴리시 (디테일)
1. **스킬 카드 등장 애니메이션** — 카드가 휘릭 돌고 짠 나타남, 도는 중 선택 불가
2. **일시정지 버튼** — 화면 우상단, 탭 시 이어하기 버튼만 있는 심플 팝업
3. **카메라 경계 제한** — 맵 밖 안 찍히게 (멀티 스테이지 고려한 유연한 구조로)

#### 게임플레이
4. **스킬 동작 검증** — 7종 스킬 실제 플레이에서 동작 확인 및 수치 조정
   - 다중 표창: 발사 수 증가 + Lv5 관통
   - 독 묻히기: 지속 데미지 + Lv5 전염
   - 분신술: 분신 위치 및 공격 확인
   - 질풍: 이동속도 + 이동 중 공격
   - 연막탄: 회피 확률 + Lv5 순간이동
   - 수리검 회전: 회전 히트박스 동작
   - 자석: 경험치 흡수 범위 확장
5. **웨이브 난이도 곡선** — 웨이브마다 적 HP/속도/데미지 단계적 강화
6. **HP 회복 아이템** — 적 처치 시 확률 드롭, 플레이어가 밟으면 회복

#### 콘텐츠
7. **로비/시작 화면** — 타이틀 UI, 게임 시작 버튼
